...
import { registration, validateUsername, validateTerpmail, removeSpaces, validatePassword } from './FirebaseMethods.js'
import { ToastMessage, ErrorAlert, styles } from '../UsefulComponents.js'
...
export default function Signup({ navigation }) {
	...
	const handleSubmit = () => {
		...
		ToastMessage('No validation signup')
		registration(username.replace(' ', ''), email.replace(' ', ''), password.replace(' ', ''))
	...
	return (
		<View style={styles.container}>
			<ScrollView keyboardDismissMode='on-drag' bounces={true}>
				...
				<Button label='Sign Up' onPress={handleSubmit} backgroundColor='#ff0000' style={styles.button} />
				<Button
					style={styles.button}
					label='Sign In'
					outline
